...
{
    public static void DecreasePrioritiesInBetween(ref BindingList<ModDataModel> installedMods, ModDataModel modChanged, int oldDisplay, int newDisplay)
    {
        foreach (ModDataModel modToDetriment in installedMods)
        {
            if (modToDetriment == modChanged)
                continue;
                
            if (modToDetriment.DisplayPriority < oldDisplay || modToDetriment.DisplayPriority > newDisplay)
                continue;
    ...
    public static void IncreasePrioritiesInBetween(ref BindingList<ModDataModel> installedMods, ModDataModel modChanged, int oldDisplay, int newDisplay)
    {
        foreach (ModDataModel modToDetriment in installedMods)
        {
            if (modToDetriment == modChanged)
            ...
            if (modToDetriment.DisplayPriority < newDisplay || modToDetriment.DisplayPriority > oldDisplay)
                continue;
